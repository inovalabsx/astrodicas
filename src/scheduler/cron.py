"""AstroDicas — Agendador de conteúdo automático."""

import asyncio
from datetime import datetime

from apscheduler.schedulers.asyncio import AsyncIOScheduler
from apscheduler.triggers.cron import CronTrigger

from src.config.settings import settings
from src.scheduler.conteudo_diario import gerar_conteudo


scheduler = AsyncIOScheduler()


async def job_publicar(tipo: str):
    """Gera e publica um conteúdo no canal Telegram."""
    try:
        conteudo = await gerar_conteudo(tipo=tipo)
        # TODO: conectar com Telegram bot para publicar
        # TODO: gerar imagem via DALL-E / Stability
        # TODO: salvar no banco (conteudos_gerados)
        print(f"[{datetime.now()}] Conteúdo gerado: {tipo}")
        print(conteudo.get("conteudo", "")[:100])
    except Exception as e:
        print(f"[ERRO] Falha ao gerar conteúdo {tipo}: {e}")


def init_scheduler():
    """Inicializa os jobs agendados."""

    horarios = [
        (settings.post_morning, "horoscopo"),
        (settings.post_afternoon, "lua"),
        (settings.post_night, "frase"),
    ]

    for horario, tipo in horarios:
        hora, minuto = horario.split(":")
        scheduler.add_job(
            job_publicar,
            trigger=CronTrigger(hour=int(hora), minute=int(minuto)),
            args=[tipo],
            id=f"post_{tipo}",
            replace_existing=True,
        )
        print(f"Agendado: {tipo} às {horario}")

    # Um post extra de trânsito nos domingos
    scheduler.add_job(
        job_publicar,
        trigger=CronTrigger(day_of_week="sun", hour=10, minute=0),
        args=["transito"],
        id="post_transito_domingo",
        replace_existing=True,
    )

    scheduler.start()
